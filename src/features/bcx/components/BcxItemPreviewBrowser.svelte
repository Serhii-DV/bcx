...
let previewBrowser: HTMLDivElement;
...
function resize(event: PointerEvent) {
  ...
  const availableHeight = previewBrowser.clientHeight;
...
<div
  bind:this={previewBrowser}
  class:resizing={isResizing}
  class="item-preview-browser"
...
>
  ...
  <section id={itemPreviewId} class="item-preview" aria-label="Selected item details">
...
<style>
.item-preview-browser { display: grid; flex: 1 1 0%; min-height: 0; overflow: hidden; }
.item-preview-browser.resizing { cursor: row-resize; user-select: none; }
.item-list, .item-preview { display: flex; flex-direction: column; min-height: 0; overflow: hidden; }
.item-resizer { display: flex; align-items: center; justify-content: center; box-sizing: content-box; width: 100%; height: 0.625rem; padding: 0; border: 0; border-block: 1px solid #4b5563; border-radius: 0; background: transparent; cursor: row-resize; touch-action: none; }
.item-resizer span { width: 2.5rem; height: 0.1875rem; border-radius: 9999px; background: #6b7280; }
.item-resizer:hover, .item-resizer:focus-visible, .item-preview-browser.resizing .item-resizer { border-color: #38bdf8; background: rgb(56 189 248 / 0.12); outline: none; }
.item-resizer:hover span, .item-resizer:focus-visible span, .item-preview-browser.resizing .item-resizer span { background: #7dd3fc; }
.item-preview-content { min-height: 0; overflow-y: auto; scrollbar-color: rgb(156 163 175 / 0.45) transparent; scrollbar-width: thin; }
